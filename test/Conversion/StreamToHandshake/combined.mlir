...
// CHECK: handshake.func private @[[LABEL_FILTER:.*]](%{{.*}}: tuple<i512, i1>, %{{.*}}: none, ...) -> (tuple<i512, i1>, none)
...
// CHECK-NEXT:  }
// CHECK-NEXT:  handshake.func private @[[LABEL_MAP:.*]](%{{.*}}: tuple<i512, i1>, %{{.*}}: none, ...) -> (tuple<i512, i1>, none)
// CHECK-NEXT:    %{{.*}}:2 = fork [2] %{{.*}} : none
// CHECK-NEXT:    %{{.*}}:2 = unpack %{{.*}} : tuple<i512, i1>
...
// CHECK-NEXT:  }
// CHECK-NEXT:  handshake.func @combined(%{{.*}}: tuple<i512, i1>, %{{.*}}: none, ...) -> (tuple<i512, i1>, none)
