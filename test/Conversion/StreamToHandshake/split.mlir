...
// CHECK: handshake.func private @stream_split(%{{.*}}: tuple<tuple<i32, i32>, i1>, %{{.*}}: none, ...) -> (tuple<i32, i1>, none, tuple<i32, i1>, none)
...
// CHECK-NEXT:  return %{{.*}}, %{{.*}}#0, %{{.*}}, %{{.*}}#1 : tuple<i32, i1>, none, tuple<i32, i1>, none
// CHECK-NEXT:}
// CHECK-NEXT:handshake.func @split(%{{.*}}: tuple<tuple<i32, i32>, i1>, %{{.*}}: none, ...) -> (tuple<i32, i1>, none, tuple<i32, i1>, none)
// CHECK-NEXT:  %{{.*}}:4 = instance @stream_split(%{{.*}}, %{{.*}}) : (tuple<tuple<i32, i32>, i1>, none) -> (tuple<i32, i1>, none, tuple<i32, i1>, none)
// CHECK-NEXT:  return %{{.*}}#0, %{{.*}}#1, %{{.*}}#2, %{{.*}}#3 : tuple<i32, i1>, none, tuple<i32, i1>, none
// CHECK-NEXT:}
