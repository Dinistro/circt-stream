...
// RUN: %PYTHON% %S/../cocotb_driver.py --objdir=%t.sv.d/ --topLevel=top --pythonModule=split-with-reg --pythonFolder=%S --testcase=all %t.sv 2>&1 | FileCheck %s
...
module {
  func.func @top(%in: !stream.stream<i64>) -> (!stream.stream<i64>, !stream.stream<i64>) {
