...
// RUN: firtool --format=mlir --verilog > %filter-export.sv && \
// RUN: circt-rtl-sim.py %filter-export.sv %S/driver_out_i64.sv %S/driver.cpp --no-default-driver --top driver | FileCheck %s
// CHECK:      Element={{.*}}1
